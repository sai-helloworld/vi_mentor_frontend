...
//     try {
//       const res = await fetch(`http://127.0.0.1:8000/api/teacher/${teacherId}/notes/`);
//       if (res.ok) {
//         const data = await res.json();
//         setUploadedNotes(data);
//       }
//     } catch (error) {
//       console.error("Failed to load previously uploaded notes.");
//     } finally {
...
//       });
//       setFiles([]);
//       if (showNotes) fetchNotes(teacherId); 
//     } else {
//       setMessage({
//         text: `⚠️ Uploaded ${successCount} files, ${failCount} failed.`,
//         type: "error",
//       });
//       if (successCount > 0 && showNotes) fetchNotes(teacherId);
//     }
//   };

//   // 4. Toggle Notes Visibility
...
//       width: "100%",
//       padding: "12px",
//       backgroundColor: "#6366f1", // Matched student page color
//       color: "white",
//       border: "none",
//       borderRadius: "6px",
//       fontSize: "1rem",
...
//       marginBottom: "10px",
//     },
//     // 📌 New Grid Styles (Matches Student Notes Page)
//     notesGrid: {
//       display: "grid",
...
//         <label style={styles.label}>Select Notes (PDF / DOCX)</label>
//         <div style={styles.fileArea}>
//           <input
//             type="file"
//             id="pdf_files"
...
//       >
//         {showNotes ? "Hide Uploaded Notes" : "Get Previously Uploaded Notes"}
//       </button>
...
//         <div>
//           {loadingNotes ? (
//             <p style={{ textAlign: "center", color: "#aaa", marginTop: "20px" }}>Loading notes...</p>
...
//                 >
//                   <img src={pdf_image} alt="PDF" style={styles.pdfIcon} />
...
//                   {/* Meta data so the teacher knows which class this belongs to */}
//                   <div style={styles.noteMeta}>
//                     {note.class_name} <br/> {note.subject_name}
//                   </div>
//                 </a>
//               ))}
//             </div>
//           ) : (
//             <p style={{ textAlign: "center", color: "#aaa", marginTop: "20px" }}>No notes uploaded yet.</p>
//           )}
//         </div>
//       )}
//     </div>
...
import React, { useState, useEffect } from "react";
// 📌 Ensure these paths are correct for your assets folder
import pdf_image from "../assets/pdf.png"; 
import word_image from "../assets/word.png"; 
...
export default function SendNotes() {
  ...
  // 📌 Helper to determine icon based on filename extension
  const getFileIcon = (filename) => {
    if (!filename) return pdf_image;
    const ext = filename.split('.').pop().toLowerCase();
    if (ext === 'docx' || ext === 'doc') return word_image;
    return pdf_image;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedAssignment || files.length === 0) {
      setMessage({ text: "Please select a class and at least one file.", type: "error" });
      return;
    }
    ...
    for (const file of files) {
      const formData = new FormData();
      // Field name remains 'pdf_file' to match your Django model, 
      // but it will hold the .docx file data.
      formData.append("pdf_file", file); 
      ...
      <form onSubmit={handleSubmit}>
        ...
        {files.length > 0 && (
          <ul style={styles.fileList}>
            {files.map((file, index) => (
              <li key={index} style={styles.fileItem}>
                <span>{file.name.endsWith('.docx') ? '📘' : '📄'} {file.name}</span>
                <button
                  type="button"
                  onClick={() => removeFile(index)}
                  style={styles.removeBtn}
            ...
            <div style={styles.notesGrid}>
              {uploadedNotes.map((note) => (
                <a
                  ...
                  style={styles.noteCard}
                >
                  {/* 📌 Display correct icon based on extension */}
                  <img 
                    src={getFileIcon(note.filename)} 
                    alt="File Icon" 
