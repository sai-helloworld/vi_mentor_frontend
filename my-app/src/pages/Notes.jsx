...
import React, { useState, useEffect } from "react";
// Ensure these assets exist in your src/assets folder
import pdf_image from "../assets/pdf.png"; 
...
const Notes = () => {
  ...
  useEffect(() => {
    ...
    if (!rollNumber) {
      setMessage("⚠️ No student roll number found. Please log in.");
      return;
    }
  ...
  // 2. Logic to determine icon based on file extension
  const getFileIcon = (filename) => {
    if (!filename) return pdf_image;
    const extension = filename.split('.').pop().toLowerCase();
    
    if (['docx', 'doc', 'dotx'].includes(extension)) {
      return word_image;
    }
    return pdf_image; // Default to PDF
  };

  // 3. Fetch Notes based on Section ID and Subject
  const fetchNotes = async () => {
    setMessage("");
    setNotes([]);
    setLoading(true);
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.header}>
        📚 Class Notes {studentName && <span style={{fontSize: "0.6em", color: "#6366f1", marginLeft: "10px"}}>for {studentName}</span>}
      </h2>

      <div style={styles.inputGroup}>
        {/* Read-only Section ID */}
        <div style={styles.inputWrapper}>
          <label style={styles.label}>Your Section</label>
          <input
            type="text"
            value={sectionId || "Loading..."}
            readOnly
        ...
        <div style={styles.notesGrid}>
          {notes.map((note) => (
            ...
            >
              ...
              />
              <div style={styles.fileName}>
                {note.filename.length > 25 
                  ? note.filename.substring(0, 22) + "..." 
                  : note.filename}
              </div>
              {note.uploaded_at && (
                <div style={styles.fileDate}>{note.uploaded_at}</div>
              )}
            </a>
          ))}
        </div>
